...
const expenseCategories = [
  { name: 'Moradia', value: 3200, color: '#8B5CF6', icon: Building },
  { name: 'Alimentação', value: 1800, color: '#F59E0B', icon: Coffee },
  { name: 'Transporte', value: 1200, color: '#3B82F6', icon: Car },
  { name: 'Entretenimento', value: 800, color: '#EC4899', icon: Smartphone },
  { name: 'Compras', value: 950, color: '#10B981', icon: ShoppingCart },
  { name: 'Saúde', value: 650, color: '#EF4444', icon: Heart },
  { name: 'Educação', value: 400, color: '#6366F1', icon: BookOpen },
  { name: 'Viagens', value: 300, color: '#14B8A6', icon: Plane },
]
...
const goals = [
  { id: 1, name: 'Casa Própria', current: 45000, target: 120000, color: '#8B5CF6', deadline: '2025-12', priority: 'high', description: 'Entrada para apartamento de 2 quartos', category: 'Imóvel' },
  { id: 2, name: 'Viagem Europa', current: 8500, target: 15000, color: '#14B8A6', deadline: '2024-07', priority: 'medium', description: 'Viagem de 15 dias pela Europa', category: 'Lazer' },
  ...
  { id: 4, name: 'Carro Novo', current: 15000, target: 45000, color: '#F59E0B', deadline: '2025-03', priority: 'low', description: 'SUV compacto 0km', category: 'Veículo' },
  { id: 5, name: 'Curso MBA', current: 3200, target: 25000, color: '#6366F1', deadline: '2024-08', priority: 'medium', description: 'MBA em Gestão Financeira', category: 'Educação' },
  { id: 6, name: 'Setup Home Office', current: 2800, target: 8000, color: '#EC4899', deadline: '2024-04', priority: 'low', description: 'Computador e móveis para escritório', category: 'Trabalho' },
]
...
const budgetCategories = [
  { name: 'Moradia', budgeted: 3500, spent: 3200, color: '#8B5CF6' },
  { name: 'Alimentação', budgeted: 2000, spent: 1800, color: '#F59E0B' },
  { name: 'Transporte', budgeted: 1500, spent: 1200, color: '#3B82F6' },
  { name: 'Entretenimento', budgeted: 1000, spent: 800, color: '#EC4899' },
  { name: 'Compras', budgeted: 800, spent: 950, color: '#10B981' },
  { name: 'Saúde', budgeted: 700, spent: 650, color: '#EF4444' },
]

const creditCards = [
  { name: 'Nubank Ultravioleta', limit: 15000, used: 3200, dueDate: '2024-02-15', color: '#8B5CF6' },
  { name: 'Itaú Personnalité', limit: 25000, used: 1800, dueDate: '2024-02-20', color: '#F59E0B' },
...
// Componente de Gráfico de Fluxo de Caixa Avançado
    ...
    <div className="w-full h-full" style={{ minHeight: height }}>
      <svg width="100%" height="100%" viewBox="0 0 100 100" className="overflow-visible">
        ...
        {[0, 20, 40, 60, 80, 100].map(y => (
          <line key={y} x1="0" y1={y} x2="100" y2={y} stroke="#E5E7EB" strokeWidth="0.2" opacity="0.3" />
        ))}
        ...
        {data.map((item, index) => {
          ...
          return (
            <g key={index}>
              ...
              {/* Month label */}
              <text x={x} y="97" textAnchor="middle" fontSize="2.5" fill="#6B7280">
                {item.month}
              </text>
            </g>
          )
...
// Componente de Gráfico de Evolução da Carteira
    ...
    <div className="w-full h-full" style={{ minHeight: height }}>
      <svg width="100%" height="100%" viewBox="0 0 100 100" className="overflow-visible">
        ...
        {[15, 30, 45, 60, 75, 85].map(y => (
          <line key={y} x1="7.5" y1={y} x2="92.5" y2={y} stroke="#E5E7EB" strokeWidth="0.2" opacity="0.3" />
        ))}
        ...
        {data.map((item, index) => {
          ...
          return (
            <g key={index}>
              ...
              <circle cx={x} cy={getY(item.rendaFixa)} r="1" fill="#3B82F6">
                <title>Renda Fixa {item.month}: {formatCurrency(item.rendaFixa)}</title>
              </circle>
              <circle cx={x} cy={getY(item.rendaVariavel)} r="1" fill="#10B981">
                <title>Renda Variável {item.month}: {formatCurrency(item.rendaVariavel)}</title>
              </circle>
              
              {/* Month labels */}
              <text x={x} y="95" textAnchor="middle" fontSize="2.5" fill="#6B7280">
                {item.month}
              </text>
            </g>
          )
...
// Componente de Gráfico de Uso dos Cartões Avançado
  ...
  return (
    <div className="w-full h-full" style={{ minHeight: height }}>
      <svg width="100%" height="100%" viewBox="0 0 100 100" className="overflow-visible">
        {/* Grid lines */}
        {[15, 30, 45, 60, 75, 85].map(y => (
          <line key={y} x1="12.5" y1={y} x2="87.5" y2={y} stroke="#E5E7EB" strokeWidth="0.2" opacity="0.3" />
        ))}
...
export default function FinancialDashboard() {
  ...
  const renderDashboard = () => (
    <div className="space-y-8">
      {/* Alerts Section */}
      {alerts.length > 0 && (
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 border border-blue-700 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-white flex items-center">
              <Bell className="w-5 h-5 mr-2 text-blue-400" />
              Alertas Importantes
            </h3>
            <span className="text-sm text-blue-300">{alerts.length} alertas</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {alerts.slice(0, 4).map((alert) => (
              <div key={alert.id} className="flex items-start space-x-3 p-3 bg-white/60 rounded-lg">
                ...
                <p className="text-sm text-gray-200 flex-1">{alert.message}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Net Worth */}
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
            <div className="p-2 bg-gradient-to-br from-emerald-900 to-emerald-800 rounded-lg">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 group">
          <div className="flex items-center justify-between mb-4">
            <div className="p-2 bg-gradient-to-br from-pink-900 to-pink-800 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Cash Flow Chart */}
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <h3 className="text-lg font-semibold text-white mb-6">Portfólio de Investimentos</h3>
            ...
            {investments.map((investment, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-800 rounded-xl hover:bg-gray-750 transition-colors">
                ...
                <div className="text-right">
                  <p className="font-semibold text-white">R$ {investment.value.toLocaleString('pt-BR')}</p>
                  <p className="text-sm text-emerald-400">{investment.return}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              return (
                <div key={index} className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-semibold text-white">{progress.toFixed(0)}%</p>
                      <div className={`text-xs px-2 py-1 rounded-full ${
                        goal.priority === 'high' ? 'bg-red-900 text-red-400' :
                        goal.priority === 'medium' ? 'bg-amber-900 text-amber-400' :
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          ...
          <div className="space-y-4">
            {insights.slice(0, 3).map((insight, index) => (
              <div key={index} className="p-4 bg-gradient-to-r from-purple-900/30 to-blue-900/30 border border-purple-800/30 rounded-xl">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-purple-400 mt-2 font-semibold">{insight.impact}</p>
                  </div>
                </div>
              </div>
            ))}
  ...
  const renderTransactions = () => (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
          <div className="relative flex-1 min-w-0">
            ...
            <input 
              ...
              className="w-full bg-gray-900 border border-gray-800 rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all"
            />
          </div>
          <button 
            onClick={() => setShowAddTransaction(true)}
            className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 flex items-center justify-center space-x-2 whitespace-nowrap shadow-lg shadow-purple-500/30"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-emerald-900 rounded-lg flex-shrink-0">
              <ArrowUpRight className="w-5 h-5 text-emerald-400" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-gray-400 text-sm">Total Receitas</p>
              <p className="text-xl font-bold text-emerald-400 truncate">R$ 9.785,50</p>
              <p className="text-xs text-emerald-400">+12.3% vs mês anterior</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-red-900 rounded-lg flex-shrink-0">
              <ArrowDownRight className="w-5 h-5 text-red-400" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-gray-400 text-sm">Total Gastos</p>
              <p className="text-xl font-bold text-red-400 truncate">R$ 5.529,80</p>
              <p className="text-xs text-emerald-400">-5.2% vs mês anterior</p>
            </div>
          </div>
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:col-span-2 lg:col-span-1 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-gray-400 text-sm">Investimentos</p>
              <p className="text-xl font-bold text-blue-400 truncate">R$ 1.000,00</p>
              <p className="text-xs text-emerald-400">Meta mensal atingida</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Transactions List */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">Todas as Transações</h3>
        ...
        <div className="space-y-3">
          {transactions.map((transaction) => {
            ...
            return (
              <div 
                key={transaction.id} 
                className="flex items-center justify-between p-3 sm:p-4 bg-gray-800 rounded-xl hover:bg-gray-750 transition-colors group cursor-pointer"
              >
                <div className="flex items-center space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-sm sm:text-base text-white truncate">{transaction.description}</p>
                ...
                <div className="text-right flex-shrink-0 ml-3">
                  ...
                  <div className={`text-xs px-2 py-1 rounded-full mt-1 ${
                    transaction.type === 'income' ? 'bg-emerald-900 text-emerald-400' :
                    transaction.type === 'investment' ? 'bg-blue-900 text-blue-400' :
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-white">R$ 9.500,00</p>
        </div>
      </div>

      {/* Budget Overview */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-emerald-900/30 to-teal-900/30 border border-emerald-800 rounded-2xl p-6">
          <div className="text-center">
            <p className="text-emerald-400 text-sm mb-2">Disponível</p>
            <p className="text-3xl font-bold text-emerald-400">R$ 3.470</p>
            <p className="text-xs text-emerald-400 mt-1">36.5% do orçamento</p>
          </div>
        </div>

        <div className="bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border border-blue-800 rounded-2xl p-6">
          <div className="text-center">
        ...
        <div className="bg-gradient-to-br from-red-900/30 to-pink-900/30 border border-red-800 rounded-2xl p-6">
          <div className="text-center">
            <p className="text-red-400 text-sm mb-2">Excedido</p>
            <p className="text-3xl font-bold text-red-400">R$ 150</p>
            <p className="text-xs text-red-400 mt-1">1.6% acima</p>
          </div>
        </div>
      </div>
      ...
      {/* Budget Categories */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <h3 className="text-lg font-semibold text-white mb-6">Categorias de Orçamento</h3>
        <div className="space-y-6">
                ...
                <div className="w-full bg-gray-800 rounded-full h-3">
                  <div 
                    className={`h-3 rounded-full transition-all duration-500 ${
                      isOverBudget ? 'bg-gradient-to-r from-red-500 to-pink-500' : ''
                    }`}
                    style={{ 
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {creditCards.map((card, index) => {
          ...
          return (
            <div key={index} className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <div 
                    className="w-4 h-4 rounded-full" 
                  ...
                  ></div>
                  <h3 className="font-semibold text-sm text-white">{card.name}</h3>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-semibold ${isHighUsage ? 'text-red-400' : 'text-white'}`}>
                  ...
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full transition-all duration-500 ${
                        isHighUsage ? 'bg-gradient-to-r from-red-500 to-pink-500' : 'bg-gradient-to-r from-emerald-500 to-teal-500'
                      }`}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-semibold text-white">R$ {card.used.toLocaleString('pt-BR')}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-400">Limite</p>
                    <p className="font-semibold text-white">R$ {card.limit.toLocaleString('pt-BR')}</p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-800">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-white">{card.dueDate}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">Uso dos Cartões (Últimos 6 meses)</h3>
          <div className="flex items-center space-x-4 text-sm">
            <div className="flex items-center">
              <div className="w-3 h-3 bg-purple-500 rounded-full mr-2"></div>
              <span className="text-gray-400">Nubank</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-amber-500 rounded-full mr-2"></div>
  ...
  const renderInvestments = () => (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
        ...
        <div className="flex items-center space-x-4">
          <button className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 flex items-center space-x-2 shadow-lg shadow-purple-500/30">
            <Plus className="w-4 h-4" />
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-400">+22.1% este ano</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-emerald-400">R$ 636</p>
              <p className="text-xs text-emerald-400">+8.5% vs mês anterior</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-blue-400">10.7%</p>
              <p className="text-xs text-blue-400">Média anual</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-pink-400">Tipos de ativos</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">Evolução da Carteira</h3>
          <div className="flex items-center space-x-4 text-sm">
            <div className="flex items-center">
              <div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
              <span className="text-gray-400">Renda Fixa</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-emerald-500 rounded-full mr-2"></div>
              <span className="text-gray-400">Renda Variável</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-purple-500 rounded-full mr-2"></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Portfolio Breakdown */}
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <h3 className="text-lg font-semibold text-white mb-6">Composição da Carteira</h3>
          <div className="space-y-4">
            {investments.map((investment) => (
              <div key={investment.id} className="p-4 bg-gray-800 rounded-xl hover:bg-gray-750 transition-colors">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-white">{investment.type}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Risco</p>
                    <p className={`font-medium ${
                      investment.risk === 'Baixo' ? 'text-emerald-400' :
                      investment.risk === 'Médio' ? 'text-amber-400' :
                      investment.risk === 'Alto' ? 'text-red-400' :
                      'text-purple-400'
                    }`}>{investment.risk}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Liquidez</p>
                    <p className="font-medium text-white">{investment.liquidity}</p>
                  </div>
                </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <h3 className="text-lg font-semibold text-white mb-6">Alocação por Tipo</h3>
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div 
                    ...
                    style={{ backgroundColor: investment.color }}
                  ></div>
                  <span className="text-sm font-medium text-white">{investment.name}</span>
                </div>
                <div className="text-right">
                  <span className="text-sm font-semibold text-white">{investment.percentage}%</span>
  ...
  const renderGoals = () => (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
        ...
        <div className="flex items-center space-x-4">
          <button 
            ...
            className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 flex items-center space-x-2 shadow-lg shadow-purple-500/30"
          >
            <Plus className="w-4 h-4" />
            <span>Nova Meta</span>
          </button>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-purple-400">Ativas</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-400">Este ano</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-blue-400">R$ 86.500</p>
              <p className="text-xs text-blue-400">Todas as metas</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-amber-400">2024-04</p>
              <p className="text-xs text-amber-400">Setup Home Office</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">Todas as Metas</h3>
          <div className="flex items-center space-x-2">
            <button className="p-2 hover:bg-gray-800 rounded-lg transition-colors">
              <Filter className="w-4 h-4 text-gray-400" />
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {goals.map((goal) => {
            ...
            return (
              <div key={goal.id} className="p-6 bg-gray-800 rounded-xl hover:bg-gray-750 transition-colors">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button className="p-2 hover:bg-red-900 rounded-lg transition-colors text-red-400">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-700">
                  <div>
                    ...
                    <p className="font-semibold text-sm text-white">R$ {remaining.toLocaleString('pt-BR')}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Mensal necessário</p>
                    <p className="font-semibold text-sm text-white">R$ {monthlyNeeded.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</p>
                  </div>
                </div>
                
                {/* Action Button */}
                <button 
                  className="w-full mt-4 py-2 rounded-lg text-sm font-medium transition-all duration-300"
                  style={{
                    background: `linear-gradient(135deg, ${goal.color}40, ${goal.color}20)`,
  ...
  const renderInsights = () => (
    <div className="space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-amber-400">Disponíveis</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-emerald-400">R$ 4.080</p>
              <p className="text-xs text-emerald-400">Por ano</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          ...
          <div className="space-y-4">
            {insights.filter(i => i.priority === 'high').map((insight) => (
              <div key={insight.id} className="p-4 bg-gradient-to-r from-red-900/30 to-pink-900/30 border border-red-800/30 rounded-xl">
                <div className="flex items-start space-x-3">
                  <div className="p-2 bg-gray-800 rounded-lg shadow-sm">
                    {insight.type === 'savings' ? (
                      <PiggyBank className="w-4 h-4 text-emerald-400" />
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-red-400 font-semibold">{insight.impact}</p>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          ...
          <div className="space-y-4">
            {insights.filter(i => i.type === 'investment').map((insight) => (
              <div key={insight.id} className="p-4 bg-gradient-to-r from-blue-900/30 to-indigo-900/30 border border-blue-800/30 rounded-xl">
                <div className="flex items-start space-x-3">
                  <div className="p-2 bg-gray-800 rounded-lg shadow-sm">
                    <TrendingUp className="w-4 h-4 text-blue-400" />
                  </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-white">Todos os Insights</h3>
          <div className="flex items-center space-x-2">
            <select className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {insights.map((insight) => (
            <div key={insight.id} className="p-4 bg-gray-800 rounded-xl hover:bg-gray-750 transition-colors">
              <div className="flex items-start space-x-3">
                <div className={`p-2 rounded-lg ${
                  insight.type === 'savings' ? 'bg-emerald-900' :
                  insight.type === 'investment' ? 'bg-blue-900' :
                  insight.type === 'warning' ? 'bg-red-900' :
                  'bg-amber-900'
                }`}>
                  {insight.type === 'savings' ? (
                    <PiggyBank className="w-4 h-4 text-emerald-400" />
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium text-sm text-white">{insight.title}</h4>
                    <div className={`text-xs px-2 py-1 rounded-full ${
                      insight.priority === 'high' ? 'bg-red-900 text-red-400' :
                      insight.priority === 'medium' ? 'bg-amber-900 text-amber-400' :
                      'bg-blue-900 text-blue-400'
                    }`}>
                  ...
                  <div className="flex items-center justify-between">
                    <p className={`text-xs font-semibold ${
                      insight.type === 'savings' ? 'text-emerald-400' :
                      insight.type === 'investment' ? 'text-blue-400' :
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400">Personalize sua experiência no MoneyFlow</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all"
              />
            </div>
            <div>
            ...
            <button className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white py-3 rounded-lg text-sm font-medium transition-all duration-300 shadow-lg shadow-purple-500/30">
              Salvar Alterações
            </button>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
              ...
              <button className="bg-emerald-900 text-emerald-400 px-3 py-1 rounded-lg text-sm">
                Ativado
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
              <div>
                <p className="font-medium text-white">Notificações de Login</p>
            ...
            <button className="w-full bg-gradient-to-r from-blue-900/30 to-indigo-900/30 border border-blue-800 text-blue-400 py-3 rounded-lg text-sm font-medium transition-all duration-300 hover:from-blue-900/50 hover:to-indigo-900/50">
              Alterar Senha
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="flex items-center space-x-3 p-3 bg-gray-800 border-2 border-purple-500 rounded-lg">
                  <Moon className="w-4 h-4 text-purple-400" />
                  <span className="text-sm text-white">Escuro</span>
                </button>
                <button className="flex items-center space-x-3 p-3 bg-gray-800 border border-gray-700 rounded-lg hover:border-gray-600 transition-colors">
            ...
            <div>
              ...
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-purple-500 rounded-full border-2 border-purple-400 cursor-pointer"></div>
                <div className="w-8 h-8 bg-blue-500 rounded-full border border-gray-700 cursor-pointer"></div>
                <div className="w-8 h-8 bg-emerald-500 rounded-full border border-gray-700 cursor-pointer"></div>
                <div className="w-8 h-8 bg-pink-500 rounded-full border border-gray-700 cursor-pointer"></div>
              </div>
            </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
              <div>
                <p className="font-medium text-white">Vencimento de Cartões</p>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
              <div>
                <p className="font-medium text-white">Insights Semanais</p>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
              <div>
                <p className="font-medium text-white">Metas Atingidas</p>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-lg font-semibold text-white">Dados & Privacidade</h3>
          </div>
          
          <div className="space-y-4">
            <button className="w-full flex items-center justify-between p-4 bg-gray-800 rounded-lg hover:bg-gray-750 transition-colors">
              <div className="flex items-center space-x-3">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-lg font-semibold text-white">Suporte & Ajuda</h3>
          </div>
          ...
          <div className="space-y-4">
            ...
            <button className="w-full flex items-center justify-between p-4 bg-gray-800 rounded-lg hover:bg-gray-750 transition-colors">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-white">Contatar Suporte</span>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-400" />
            </button>
            ...
            <button className="w-full flex items-center justify-between p-4 bg-gray-800 rounded-lg hover:bg-gray-750 transition-colors">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-white">Avaliar App</span>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-400" />
            </button>
            
            <div className="pt-4 border-t border-gray-800">
              <button className="w-full flex items-center justify-center space-x-3 p-4 bg-gradient-to-r from-red-900/30 to-pink-900/30 border border-red-800 text-red-400 rounded-lg hover:from-red-900/50 hover:to-pink-900/50 transition-all duration-300">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center space-x-2">
            ...
            <h3 className="text-lg font-semibold text-white">MoneyFlow</h3>
          </div>
          <p className="text-sm text-gray-400">Versão 2.1.0</p>
          <p className="text-xs text-gray-500">© 2024 MoneyFlow. Todos os direitos reservados.</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      {/* Header */}
      <header className="border-b border-gray-800 bg-black/80 backdrop-blur-xl sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="w-8 h-8 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg flex items-center justify-center shadow-lg shadow-purple-500/30">
                <Droplets className="w-5 h-5 text-white" />
              </div>
              <h1 className="text-xl font-geist-sans font-semibold text-white">MoneyFlow</h1>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              <select 
                ...
                className="bg-gray-900 border border-gray-800 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all"
              >
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
                    activeTab === tab.id
                      ? 'bg-gradient-to-r from-purple-900/50 to-blue-900/50 text-purple-400 border border-purple-800/50 shadow-sm'
